 ...
 void ts_section_destroy_section(ts_section_t* section)
{
	while (section)
	{
		ts_section_t* next = section->next;
...
tb_bool_t ts_section_valid_section(ts_section_t* section)
{
	...
	if (section->section_syntax_indicator)
	{
		...
		{
			tb_trace("Bad CRC_32 (0x%08x) !!!", crc);
			return TB_FALSE;
		}
	}
	else
...
void ts_section_build_section(ts_section_t* section)
{
	...
	// setion_syntax_indicator | private_indicator | first 4 MSB of section_length
	section->data[1] = (section->section_syntax_indicator ? 0x80 : 0x00)
							| (section->private_indicator ? 0x40 : 0x00)
							| 0x30 /* reserved bits set to 1 */
							| ((section->section_length >> 8) & 0x0f);

	// 8 LSB of section_length
	section->data[2] = section->section_length & 0xff;

	// optional part of a PSI section
	if (section->section_syntax_indicator)
	{
		// 8 MSB of table_id_extension
		section->data[3] = (section->table_id_extension >> 8) & 0xff;
		// 8 LSB of table_id_extension
		section->data[4] = section->table_id_extension & 0xff;
		// 5 bits of version_number | current_next_indicator
		section->data[5] =   0xc0 /* reserved bits set to 1 */
								| ((section->version_number & 0x1f) << 1)
								| (section->current_next_indicator ? 0x01 : 0x00);
		// section_number
		section->data[6] = section->i_number;
		// last_section_number
		section->data[7] = section->i_last_number;

		// CRC_32
		section->crc = 0xffffffff;

		while (p < section->payload_end)
		{
			section->crc = (section->crc << 8) ^ s_crc32_table[(section->crc >> 24) ^ (*p)];
			p++;
		}

		section->payload_end[0] = (section->crc >> 24) & 0xff;
		section->payload_end[1] = (section->crc >> 16) & 0xff;
		section->payload_end[2] = (section->crc >> 8) & 0xff;
		section->payload_end[3] = section->crc & 0xff;

		// valid section
		tb_assert(TB_TRUE == ts_section_valid_section(section);
	}
}
#endif
...
static tb_bool_t ts_section_impl_continuity_check(ts_packet_t* ts_packet, ts_section_decoder_t* sc_decoder)
{
	...
	{
		tb_trace("ts duplicate (expected: %d, received: %d) for pid %d"
	...
	{
		tb_trace("ts discontinuity (expected: %d, received: %d) for pid %d"
			  , sc_decoder->continuity_counter, ts_header->continuity_counter, ts_header->pid);
		...
		{
			ts_section_destroy_section(sc_decoder->current_section);
...
void ts_section_decode(ts_packet_t* ts_packet, ts_section_decoder_t* sc_decoder)
{
	...
	if (ts_header->payload_unit_start_indicator)
		new_section = payload_data;

	// if the decoder needs a begginning of section and a new section
	// begins in the packet then initialize the section
	if (!section)
	{
		// cannot find start packet, continue
	...
	// process remaining_bytes
	while (remaining_bytes > 0)
	{
		// there are enough bytes in this packet to complete the header/section
		if (remaining_bytes >= sc_decoder->processed_size)
		{
			// append data to section
			memcpy(section->payload_end, payload_data, sc_decoder->processed_size);
			payload_data			+= sc_decoder->processed_size;
			section->payload_end	+= sc_decoder->processed_size;
			...
			else	// process the whole section
			{
				...
				if (TB_TRUE == ts_section_valid_section(section))
				{
					...
					else
					{
						...
						// points immediately after the section_length field
						section->payload_start				= section->data + 3;
					}

					// decode section table
					if (sc_decoder->callback) sc_decoder->callback(ts_packet, sc_decoder);

					// current section has been processed, reset it
					sc_decoder->current_section = NULL;
				}
				else
				{
					tb_trace("invalid section(pid:%x)", ts_header->pid);
				...
				if (new_section == NULL && remaining_bytes && *payload_data != 0xff)
					new_section = payload_data;
		...
		else
		{
			...
			memcpy(section->payload_end, payload_data, remaining_bytes);
			section->payload_end		+= remaining_bytes;
			sc_decoder->processed_size	-= remaining_bytes;
			remaining_bytes				= 0;
		}
	}
}
