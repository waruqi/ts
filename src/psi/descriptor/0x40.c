...
ts_network_name_descriptor_t* ts_network_name_descriptor_decode(ts_descriptor_t* dr)
{
	...
	{
		TS_DBG("network name descriptor: bad tag (%#x)", dr->descriptor_tag);
		return NULL;
	}
	...
	{
		TS_DBG("network name descriptor: bad length (%d)", dr->descriptor_length);
		return NULL;
	}
	...
	if (!dr->data)
	{
		TS_DBG("network name descriptor: invalid data");
		return NULL;
	}
	...
	if (!p)
	{
		TS_DBG("network name descriptor: out of memory");
		return NULL;
	}
	...
	// decode data
	ts_size_t n = dr->descriptor_length;
